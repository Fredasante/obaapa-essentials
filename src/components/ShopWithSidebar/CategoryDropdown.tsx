...
const CategoryItem = ({ category, selected, onToggle }) => {
  return (
    <button
      ...
      className={`${
        selected ? "text-seaBlue-dark" : "text-dark"
      } group flex items-center justify-between ease-out duration-200 hover:text-seaBlue-dark`}
    >
      <div className="flex items-center gap-2">
        <div
          className={`cursor-pointer flex items-center justify-center rounded-full w-4 h-4 border ${
            selected ? "border-seaBlue-dark" : "border-gray-3"
          }`}
        >
          {selected && <div className="w-2 h-2 rounded-full bg-seaBlue-dark" />}
        </div>
        <span>{category.displayName || category.name}</span>
      </div>
      <span
        className={`${
          selected ? "text-white bg-seaBlue-dark" : "bg-gray-2 text-dark"
        } inline-flex rounded-[30px] text-custom-xs px-2 ease-out duration-200 group-hover:text-white group-hover:bg-seaBlue-dark`}
      >
        {category.products}
      </span>
    </button>
